...
| 配置事件订阅 | 请求地址填 OpenClaw gateway 的 webhook 地址，订阅 `im.message.receive_v1` |
| 发布应用 | 提交审核并发布 |
...
通过 OpenClaw 插件系统安装，自动创建 `~/.openclaw/extensions/feishu/` 目录。安装后无需手动修改。
...
```jsonc
{
  "models": {
    ...
    "providers": {
      ...
      // 如果使用 OpenRouter，在此新增 OpenRouter provider
    }
  }
}
```

### 4.3 agents — Agent 列表（关键）
...
```jsonc
{
  "agents": {
    "defaults": {
      "model": {
        "primary": "xiaomi/mimo-v2-pro"    // 默认模型
      },
      "models": {
...
- [ ] `openclaw.json` 中 `models.providers` 包含所需模型
