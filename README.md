...
通过 `openclaw agents add` 添加 rd 和 pm agent（main 通常已由初始化创建）：

```bash
# 添加 rd agent
openclaw agents add rd \
  --workspace ~/.openclaw/workspace-rd \
  --name "研发小工" --emoji "🔧"
...
该命令会自动：
- 在 `openclaw.json` 的 `agents.list` 中创建 agent 条目
- 创建 workspace 目录

> **注意**：`openclaw agents add` 创建的 agent 条目默认不含 `tools.allow`。需要在 `openclaw.json` 中手动给每个 agent 补上 `"tools": { "allow": ["*", "group:plugins"] }`。详见第四节说明。
...
- [ ] 通过 `openclaw agents add` 添加 rd 和 pm agent
- [ ] `openclaw.json` 中每个 agent 的 `tools.allow` 含 `"*"`
